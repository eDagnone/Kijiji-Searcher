...
function parseResultsHTML(html) {
    ...
    //Everything up to here works. Fix the below, and submit MR to Kijiji-scraper repo.
    ...
    if (scriptTag.length > 0) {
        ...
        const listings = [];
        for (const prop in jsonData.props.pageProps.__APOLLO_STATE__) {
            if (prop.match(/ListingV2:.*/)) {
                const listing = jsonData.props.pageProps.__APOLLO_STATE__[prop];
                let url = listing.seoUrl;
                let info = {
                    "title": listing.title,
                    "image": listing.imageUrls[0] || "NONE",
                    "images": listing.imageUrls
                    "date": listing.activationDate,
...
function initFirstResultPagePath(params, firstResultPagePath="") {
    return new Promise(function(resolve, reject) {
        ...
        const responseText = fs.readFileSync('./new_html.txt', 'utf8');
...
function getPageResults(params, pageNum, firstResultPagePath="") {
    const responseText = fs.readFileSync('./new_html.txt', 'utf8');
    let results = parseResultsHTML(responseText);
    return {pageResults: results, firstResultPagePath: "localhost"};
    ...
    /* When searching with formSubmit = true, Kijiji will redirect us to a URL
       ...
       return initFirstResultPagePath(params, firstResultPagePath).then(function(path) {
        ...
        if (!results)
            throw new Error("Invalid Kijiji HTML on search results page " + pageNum);
